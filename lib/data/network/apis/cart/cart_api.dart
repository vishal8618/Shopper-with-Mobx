...
class CartApi {
  ...
  Future<UpdateCartModel> addCart({String uid, String productId}) async {
    try {
      ...
      return updateCartFromJson(json.encode(res.data));
    } catch (e) {
      print(e.toString());
      throw e;
    }
  ...
  Future<UpdateCartModel> removeCart({String uid, String productId,String quantity}) async {
    try {
      var map = HashMap<String, dynamic>();
      map["buyer_id"] = int.parse(uid);
      map["product_id"] = int.parse(productId);
      ...
      // to delete item
      if(quantity!=null)
      map["item_quantity"] = int.parse(quantity);
